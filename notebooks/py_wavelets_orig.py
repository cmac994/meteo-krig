...
def cwt(signal, dt, dj=1/12, s0=-1, J=-1, pad=0., wavelet='morlet', freqs=None, normalize=False):
    ...
    if normalize:
        ...
        signal = (signal - signal.mean()) / std1
    else:
        signal = signal
    ...
    #    signal=np.concatenate((signal,temp))
...
def icwt(W, sj, dt, dj=1/12, wavelet='morlet'):
    ...
    iW = dj * np.sqrt(dt) / wavelet.cdelta /wavelet.psi(0) * (np.real(W) / sj**0.5).sum(axis=0)
    return iW
...
def wcct(y1, y2, dt, dj=1/12, s0=-1, J=-1, ln=72, s1=2, s2=6, sig=True,
        significance_level=0.95, wavelet='morlet', normalize=True, **kwargs):
    """Wavelet cross correlation transform (WCCT).

    The WCCT finds regions in time frequency space where the two time
    series co-vary, but do not necessarily have high power.
    ...
    # Normalizes both signals, if appropriate.
    if normalize:
        #y1_normal = detrend(y1)
        #y2_normal = detrend(y2)
        std1 = y1.std()
        std2 = y2.std()
        y1_normal = (y1 - y1.mean()) / std1
        y2_normal = (y2 - y2.mean()) / std2
    ...
    #y1_padded = np.zeros(y1.shape[0]*2)
    y2_padded = np.pad(y2_normal.conj(),2*ln,mode='constant') #e.g. len: 432
    #y1_padded[ln:ln+len(y1_normal)] = y1_normal
    #y2_padded[ln:ln+len(y2_normal)] = y2_normal

    #ps = [i + j  for i, j in zip(y1_normal.shape, y2_normal.shape)]
    # zero pad input
    y1_padded = np.pad(y1, ln, mode='constant') #e.g. len: 288

    #print(y2_padded.shape)
    #print(y1_padded.shape#)

    #y2_padded = y2_normal.copy()
    #y1_padded = np.zeros(ps, y1_normal.dtype)
    #sc = tuple(slice(0, i) for i in y1_normal.shape)
    #y1_padded[sc] = y1_normal.copy()

    #y2_padded = np.zeros(ps, y2_normal.dtype)
    #sc = tuple(slice(0, i) for i in y2_normal.shape)
    #y2_padded[sc] = y2_normal.copy()
   
    # Calculates the CWT of the time-series making sure the same parameters
    # are used in both calculations.
    _kwargs = dict(dj=dj, s0=s0, J=J, wavelet=wavelet)

    #print(len(y1_normal[ln:-ln]))
    W1, sj, freq0, coi0, _, _ = cwt(y1_padded, dt, **_kwargs)
    W2, sj, freq0, coi0, _, _ = cwt(y2_padded[2*ln-ln:-ln],dt,**_kwargs)
    
    #W1 /= sj[:,None]
    #W2 /= sj[:,None] 
    #W1, sj, freq, coi, _, _ = cwt(y1_normal[l1:l2], dt, **_kwargs)
    #W2, sj, freq, coi, _, _ = cwt(y2_normal[l1:l2], dt, **_kwargs)
    #wave, scales, freqs, coi, fft, fftfreqs
 
    #cent = int(len(y2_padded)/2)
    #mll = cent - ln
    #mlu = cent + ln
    ...
    #print(str(s11)+','+str(s22)+','+str(period.shape)+','+str(W1.shape))
    #W1 = W1[s11:s22,:]
    #W2 = W2[s11:s22,:]
    ...
    #ang1 = np.angle(W1)
    #ang2 = np.angle(W2)
    #scales1 = np.ones([1, y1[l1:l2].size]) * sj[:, None]
    #scales2 = np.ones([1, y2[l1:l2].size]) * sj[:, None]

    # Now the wavelet transform coherence
    #W1prime = []; W2prime = []
    #for s in range(0,W1.shape[0]):
    #    W1prime.append(W1[s,:] - np.mean(W1[s,:])) #/np.std(W1[s,:])
    #    W2prime.append(W2[s,:] - np.mean(W2[s,:])) #/np.std(W2[s,:])

    #W1prime = np.array(W1prime); W2prime = np.array(W2prime)
    #W1prime = W1prime[s1:s2,:]
    #W2prime = W2prime[s1:s2,:]
    #W1 = W1[s1:s2,:]
    ...
    #print('mlu: '+str(mlu))
    WCC = []; lags = []; WCC_max = []
    for i in range(0,2*ln): #int(-(l2-l1)/2),int((l2-l1)/2) + 1):
        #print(ln+lag)
        #print(-ln+lag)
       	#print(str(lag)+' | '+str(mll+lag)+': '+str(mlu+lag))
        if (i == 0):
           ytmp = y2_padded[2*ln-i:]
        else:
           ytmp = y2_padded[2*ln-i:-i]

        W2t, sj, freq, coi, _, _ = cwt(ytmp, dt, **_kwargs)
        W2t = np.array(W2t)

        #W2t /= np.sqrt(sj[:,None])
        #W2tprime = []
        #for k in range(0,W2t.shape[0]):
        #    W2tprime.append(W2t[k,:] - np.mean(W2t[k,:])) #/np.std(W2t[k,:])
        #W2tprime = np.array(W2tprime)

        #W2tprime = W2tprime[s1:s2,:]
        #print("W2tau': "+str(W2tprime))
        #W2t = W2t[s1:s2,:]
        #W2t = W2t[s11:s22,:]
        W12 = W1.conj()*W2t #.conj()
        #print(W12.shape)
        WCC_max.append(np.sum(np.sum(W12[s11:s22,:],axis=1))) #np.sum(np.sum(W12,axis=1)))
        #WCC.append( np.real(np.sum(W12,axis=1)) / (np.real(np.sum(np.abs(W1)**2,axis=1))*np.real(np.sum(np.abs(W2t)**2,axis=1)))**0.5)
        WCC.append( np.real(np.sum(W12,axis=1)) / (np.real(np.sum(np.abs(W1)**2,axis=1))*np.real(np.sum(np.abs(W2)**2,axis=1)))**0.5)
        lags.append(-ln+i)
    
    #freq0 = freq0[s11:s22]
    return np.array(WCC), np.array(WCC_max), np.array(lags), np.array(freq0), np.array(coi0)
